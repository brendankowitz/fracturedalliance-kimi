...
function IsoBox({ cx, cy, w, d, h, top = TILE_TONES.top, sideR = TILE_TONES.sideR, sideL = TILE_TONES.sideL, stroke = TILE_TONES.edge }: {
  cx: number | string; cy: number | string; w: number | string; d: number | string; h: number | string;
  top?: string; sideR?: string; sideL?: string; stroke?: string;
}) {
  const x = Number(cx), y = Number(cy), hw = Number(w), hd = Number(d), hh = Number(h);
  const fb = [x, y];
  const rb = [x + hw, y - hw / 2];
  const lb = [x - hd, y - hd / 2];
  void [x + hw - hd, y - hw / 2 - hd / 2];
  const ft = [x, y - hh];
  const rt = [x + hw, y - hw / 2 - hh];
  const lt = [x - hd, y - hd / 2 - hh];
  const bt = [x + hw - hd, y - hw / 2 - hd / 2 - hh];
  return (
    <g>
...
function IsoPad({ cx, cy, w, d, fill = '#161b25', stroke = TILE_TONES.edge }: {
  cx: number | string; cy: number | string; w: number | string; d: number | string; fill?: string; stroke?: string;
}) {
  const x = Number(cx), y = Number(cy), hw = Number(w), hd = Number(d);
  return (
    <polygon
      points={`${x},${y} ${x + hw},${y - hw / 2} ${x + hw - hd},${y - hw / 2 - hd / 2} ${x - hd},${y - hd / 2}`}
      fill={fill} stroke={stroke} strokeWidth="0.3"
    />
  );
}

export function BuildingTile({ id, size = 56, glow = true }: { id: string; size?: number; glow?: boolean }) {
