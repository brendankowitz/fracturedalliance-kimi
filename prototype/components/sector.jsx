...
function SectorMap({ onJumpToColony }) {
  const [selectedId, setSelectedId] = React.useState('forge-3');
  const asteroids = window.GameData.ASTEROIDS;
  const selected = asteroids.find(a => a.id === selectedId);
  const owner = window.GameData.RACES.find(r => r.id === selected?.ownerId);

  return (
    <div className="screen screen-enter" style={{ display: 'grid', gridTemplateColumns: '1fr 380px', height: '100%' }}>
      <SectorCanvas asteroids={asteroids} selectedId={selectedId} onSelect={setSelectedId} />
      <SectorInspector asteroid={selected} owner={owner} onJumpToColony={onJumpToColony} />
    </div>
  );
}

function SectorCanvas({ asteroids, selectedId, onSelect }) {
  return (
...
function SectorInspector({ asteroid, owner, onJumpToColony }) {
