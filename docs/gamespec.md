...
**Design source of truth.** A complete clickable UI prototype — `prototype/Fractured Alliance.html` — was created by Claude Designer (claude.ai/design) and serves as the canonical reference for screen layouts, colour palette, typography, iconography, and animation vocabulary. The prototype implements eight navigable screens (Main Menu, Sector Map, Colony, Sci-Tek, Diplomacy, Commerce, Tactical, Black Cell) with a "Helion Corp Operations Console" aesthetic: cool charcoal stack, amber signal accents, Space Grotesk + JetBrains Mono typography, and a full asset library of schematic glyphs, isometric animated building tiles, ship silhouettes, and ordnance icons. All implementation UI work must align with this prototype.
...
**Stability.** Save-rename crash; `_MA\_MA` load crash on some DOSBox configs; Alt-Tab crashes on Windows 95 build; no autosave; IPX networking broken in every modern wrapper.
...
**Save system.** IndexedDB multi-slot with autosave every sim-minute and quicksave. Save files are JSON + gzip with schema versioning and forward migrations. Export/import save to file for sharing and bug reports.

**Telemetry.** Opt-in (off by default), Do-Not-Track respected, no PII. Events: anonymous session length, final score/verdict, blueprints bought in order, first-death cause, AI difficulty setting. **No third-party ad SDKs, ever.**
...
**Reach.** PWA-installable, offline-capable. **Tauri 2.0** desktop wrapper for Steam Deck and optional Steam shipping. Touch-first controls on tablets (pinch-zoom, long-press context). Gamepad API for controller play.
...
| **Meta** (across matches) | 10+ matches | Unlock ambient cosmetics, weekly seeds, optional persistent megacorp reputation |
...
> The architecture is entirely client-side. The game is a PWA with IndexedDB persistence — no server.
...
**Why fixed timestep.** Determinism. Replays, saves, and bug reports all require that the same inputs on the same seed produce the same state. Variable timestep makes that essentially impossible.
...
├── tsconfig.base.json
├── pnpm-workspace.yaml
└── .github/workflows/        # CI: web build, tests
```
...
**Risk — feature creep from community requests (map editor, real 3D).** **Mitigation:** freeze the Phase 2 content list in a versioned `scope.md`; require a written "next-phase ticket" for any deferred request; keep a stretch-goals.md separate from the roadmap; ship an MVP before admitting *any* stretch feature.
...
**Risk — browser-only stigma.** Some hardcore strategy fans dismiss browser games. **Mitigation:** ship the Tauri desktop + Steam build alongside from Phase 3; make the web version the playable demo, charge for the desktop/Steam SKU, with full feature parity.
